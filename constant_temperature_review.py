...
# Implementation is for a constant effective temperature
# This is in response to the reviewer asking what if the system qubit is not perfectly isolated
''' 
The Pauli matrices
'''
...
'''
sz_list = manqsp.qm_sparse(n, sz_pauli) # n - 1 for previous implementation
sp_list = manqsp.qm_sparse(n, sp_pauli) # n - 1 for previous implementation
sm_list = manqsp.qm_sparse(n, sm_pauli) # n - 1 for previous implementation
...
hint_plus = manqsp.kron_sys_ancilla(sy.sparse.csr_array(sp_pauli), manqsp.qm_sparse(n-1, sm_pauli))
hint_minus = manqsp.kron_sys_ancilla(sy.sparse.csr_array(sm_pauli), manqsp.qm_sparse(n-1, sp_pauli))

'''
5. Putting the values of the constants and generating the frequencies of the ancilla qubits
'''
Te = 1.65 # for fig. 5 : Te = 1.5
l, T0, dt = 0.001, 1, 0.1 # for fig. 5 : l, T0, dt = 0.001, 1, 0.1
t = np.arange(0, 200, dt)
theta = np.zeros(n - 1)
omi = np.linspace(1.1, 2, n - 1) # for fig. 5 : omi = np.linspace(1.1, 2, n - 1)
rate_args = [theta, l, Te]
efft_args = [T0]
o = sy.integrate.odeint(manqdense.omegasine, omi, t, args=(rate_args,  efft_args))

o_system = 1

g = (0.2/(n - 1)) * np.ones(n - 1) # for fig. 5 : g = (0.2/(n - 1)) * np.ones(n - 1)
#g = 0.055 * np.ones(n - 1)
dimq = 2 * np.ones((2, n))

g_sys = l # coupling strength of the system ancilla to the environment
rate_args.append(g_sys)
...
#rho_sys = np.array([[1/3, np.sqrt(2)/3], [np.sqrt(2)/3, 2/3]])
...
all_matrices = [sp_list, sm_list, sz_list, hint_plus, hint_minus, ham]
for i in range(len(t) - 1):
    strt_time = time.time()

    L_rho = manqsp.Lind_action_with_sys_bath(o[i+1, :], rho, all_matrices, o_system, g, rate_args)
    L2_rho = manqsp.Lind_action_with_sys_bath(o[i+1, :], L_rho, all_matrices, o_system, g, rate_args)
    L3_rho = manqsp.Lind_action_with_sys_bath(o[i+1, :], L2_rho, all_matrices, o_system, g, rate_args)
    L4_rho = manqsp.Lind_action_with_sys_bath(o[i+1, :], L3_rho, all_matrices, o_system, g, rate_args)
    L5_rho = manqsp.Lind_action_with_sys_bath(o[i+1, :], L4_rho, all_matrices, o_system, g, rate_args)
    ...
    q_rho = qp.Qobj(rm, dims = [[2, 2, 2, 2, 2, 2, 2, 2], [ 2, 2, 2, 2, 2, 2, 2, 2]])
    for j in range(n):
...
with open('frequencies_fig_1.npy', 'wb') as f:
    np.save(f, t)
    np.save(f, o)

with open('temperatures_fig_1.npy', 'wb') as g:
    np.save(g, T0 * np.ones(len(t)))
...
axs[0].legend()
axs[1].plot(t, T0 * np.ones(len(t)), color ='#DE9E46', label = 'effective_temp')
axs[1].plot(t, temp_ancillas[:, 0], color = '#2D8077', linewidth = 2.5, label = 'system_qubit_temp')
axs[1].plot(t, Te * np.ones(len(t)), color = '#CD4F41', linewidth = 2.5, label = 'Te')
